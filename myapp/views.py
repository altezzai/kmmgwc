...
def index(request):
    ...
    return render(
        request, "index.html", {"events": evt, "news": nws, "principal": principal}
    )
...
def faculty(request, dept):
    ...
    employees = Employee.objects.filter(department=department).order_by(
        "seniority", "name"
    )
...
def office(request):
    ...
    office_staff = sorted(
        office_staff, key=lambda emp: position_order.get(emp.position, 999)
    )
...
def statement(request):
    statements = StatementCompliance.objects.all().order_by(
        "-id"
...
def student_satisfaction(request):
    ...
    return render(
        request, "student_satisfaction.html", {"entries": student_satisfaction_entries}
    )
    ...
    if "username" in request.session:  # Ensure the user is logged in
        if request.method == "POST":
            try:
                ...
                total_work_experience = request.POST.get(
                    "total_work_experience", ""
                ).strip()
                seminars_conferences_organised = request.POST.get(
                    "seminars_conferences_organised", ""
                ).strip()
                ...
                additional_responsibilities = request.POST.get(
                    "additional_responsibilities", ""
                ).strip()
                phd_mphil_projects_guided = request.POST.get(
                    "phd_mphil_projects_guided", ""
                ).strip()
                major_minor_projects = request.POST.get(
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(
                    ...
                    {"calendar": calendar, "errors": form.errors},
                )
...
def create_iqac_member(request):
    ...
    if "username" in request.session:
        if request.method == "POST":
            form = IQACMemberForm(request.POST)
            if form.is_valid():
                form.save()
...
def update_iqac_member(request, id):
    ...
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = IQACMemberForm(request.POST, instance=member)
            if form.is_valid():
                form.save()
                return redirect("iqac_member_list")
            else:
                return render(
                    request,
                    "update_iqac_member.html",
                    {"member": member, "errors": form.errors},
                )
...
def create_iqac_minute(request):
    if "username" in request.session:
        if request.method == "POST":
            form = IQACMinuteForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
...
def update_iqac_minute(request, id):
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = IQACMinuteForm(request.POST, request.FILES, instance=minute)
            if form.is_valid():
                form.save()
                return redirect("iqac_minutes_list")
            else:
                return render(
                    ...
                    {"minute": minute, "errors": form.errors},
                )
...
def statement_compliance_list(request):
    if "username" in request.session:
        statements = StatementCompliance.objects.all().order_by("-id")
        return render(
            request, "statement_compliance_list.html", {"statements": statements}
        )
    return redirect("login")
...
def create_statement_compliance(request):
    if "username" in request.session:
        if request.method == "POST":
            form = StatementComplianceForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
...
def update_statement_compliance(request, id):
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = StatementComplianceForm(request.POST, request.FILES, instance=statement)
            if form.is_valid():
                form.save()
                return redirect("statement_compliance_list")
            else:
                return render(
                    ...
                    {"statement": statement, "errors": form.errors},
                )
...
def aqar(request):
    ...
    return render(
        request, "aqar.html", {"aqar_list": aqar_list, "aqar_reports": aqar_reports}
    )
...
def create_aqar(request):
    if "username" in request.session:
        if request.method == "POST":
            form = AQARForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
...
def update_aqar(request, id):
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = AQARForm(request.POST, request.FILES, instance=aqar)
            if form.is_valid():
                form.save()
                return redirect("aqar_list")
            else:
...
def create_aqar_report(request):
    if "username" in request.session:
        if request.method == "POST":
            form = AQARReportForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
...
def update_aqar_report(request, id):
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = AQARReportForm(request.POST, request.FILES, instance=aqar_report)
            if form.is_valid():
                form.save()
                return redirect("aqar_report_list")
            else:
                return render(
                    ...
                    {"aqar_report": aqar_report, "errors": form.errors},
                )
...
def create_aishe(request):
    if "username" in request.session:
        if request.method == "POST":
            form = AISHEForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
...
def update_aishe(request, id):
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = AISHEForm(request.POST, request.FILES, instance=aishe_record)
            if form.is_valid():
                form.save()
                return redirect("aishe_list")
            else:
                return render(
                    request,
                    "update_aishe.html",
                    {"aishe_record": aishe_record, "errors": form.errors},
                )
...
def best_practice_list(request):
    if "username" in request.session:
        best_practices = BestPractice.objects.all().order_by("-id")
        return render(
            request, "best_practice_list.html", {"best_practices": best_practices}
        )
    return redirect("login")
...
def create_best_practice(request):
    if "username" in request.session:
        if request.method == "POST":
            form = BestPracticeForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
...
def update_best_practice(request, id):
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = BestPracticeForm(request.POST, request.FILES, instance=best_practice)
            if form.is_valid():
                form.save()
                return redirect("best_practice_list")
            else:
                return render(
                    ...
                    {"best_practice": best_practice, "errors": form.errors},
                )
...
def student_satisfaction_list(request):
    ...
    return render(
        request,
        "student_satisfaction_list.html",
        {"student_satisfactions": student_satisfactions},
    )
...
def create_student_satisfaction(request):
    if "username" in request.session:
        if request.method == "POST":
            form = StudentSatisfactionForm(request.POST, request.FILES)
            if form.is_valid():
                form.save()
...
def update_student_satisfaction(request, id):
    if "username" in request.session:
        ...
        if request.method == "POST":
            form = StudentSatisfactionForm(
                request.POST, request.FILES, instance=student_satisfaction
            )
            if form.is_valid():
                form.save()
                return redirect("student_satisfaction_list")
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(
                    ...
                    {"feedback": feedback, "errors": form.errors},
                )
...
def list_notifications(request):
    if "username" in request.session:
        ...
        return render(
            request, "notification_list.html", {"notifications": notifications}
        )
    return redirect("login")
